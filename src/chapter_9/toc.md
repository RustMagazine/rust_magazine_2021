# 九月刊（September）

## 发刊通告

- [发刊通告](./announce.md)

## Rust 资讯
- [官方动态](./lang.md)
- [社区热点](./hots.md)
- [活动回顾](./events.md)
- [本月招聘](./jobs.md)
- [官方 RustConf 2021 盘点](./rustconf.md)
- [RustChinaConf 2021 动态](./rust_china_conf_2021.md)


## 外刊评论

- [将 TensorFlow 模型移植到 Pure Rust 的开发成本有多高？](./foreign_journal_review.md)
- [Rust 2021 有哪些新功能](./foreign_journal_review.md)
- [周期性服务发现](./foreign_journal_review.md)

## Rust 生产实践

- [Datenlord | 重新思考 Rust Async - 如何实现高性能 I/O](./rethink-async.md)
- [Phala Network | 在Intel SGX环境下实现Rust原生std支持](./intel_sgx_std.md) 
- [如何用Rust做Android UI渲染](./rust-android-ui.md)

## 学习园地

### 新手入门

- [学习资源](./learn.md)
- [Rust 生命周期与智能指针](./lifetime-smart-ptr.md) 
- [为 嵌入式 C  程序员编写的 Rust 指南](./rust-for-embedded-c.md)

### 原理与工程实践

- [可视化项目成员包的调用关系](./visualize_crate_graph.md)

## 开源项目巡礼

- [基于 Poem 的 OpenAPI 服务端框架](./poem-openapi.md)
- [密码协议算法与Rust生态](./rust-cryptography-ecosystem.md)
- [GitHub 趋势榜](./github_trending.md)
- [推荐项目 ｜ 基础工具库](./tool_libs.md)
- [推荐项目 |  框架引擎](./frameworks.md)

##  Rust 与 安全

- [Rust生态安全漏洞总结系列 | Part 3](./rust-security-part3.md)
