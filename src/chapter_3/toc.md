# 三月刊

![logo](../image/rust_magazine3.png)


### 发刊通告

- [发刊通告](./announce.md)

### 本月社区动态简报

精选自《Rust日报》

- [Rust官方动态](./lang.md)
- [社区热点](./hots.md)
- [推荐项目](./projects.md)
- [学习资源](./learns.md)
- [Rust 唠嗑室本月汇总](./rust_laoke.md)

### Rust in Production 

- [华为 | 基于Rust的下一代虚拟化平台-StratoVirt](./hw_rust_stratovirt.md)
- [华为 | Rust 科学计算多维数组运算库的分析与实践](./hw_ndarray.md)
- [华为 | 基于 TVM Rust Runtime 和 WASM 沙箱运行 AI 模型](./hw_rust_rvm_wasm_ai.md)
- [蚂蚁集团 CeresDB 团队 | Rust CPU 亲和性(Affinity) 初探](./rust_cpu_affinity.md)
- [DatenLord | Rust实现RDMA](./rust_rdma.md)

### 学习园地

- [`no_std` 环境下的可执行文件](./no_std_binary.md)
- [用 Rust 写智能合约 | Hello, Ink! ](./ink.md)
- [「算法」蓄水池算法改进 - 面向抽奖场景保证等概率性](./reservoir.md)
- [Rust 中使用 MySQL](./rust-mysql.md)
- [「系列」Rust 设计模式 ｜ 工厂模式](./rust-design-pattern-factory.md)
- [「译」数据操作：Rust vs Pandas](./rust_vs_pandas.md)
- [「译」Unsafe Rust 的取舍](./Unsafe_Rust_How_and_when_not_to_use_it.md)
- [「译」基于 Rust 用 Bevy 实现节奏大师游戏](./Rhythm-game-in-Rust-using-bevy.md)
- [「译」Arenas in Rust](./arenas-in-rust.md)
- [「译」用 Rust 编写 LLVM 的玩具编译器](./toy-front-end-for-llvm-write-in-rust.md)


### 【系列】透过 Rust 探索系统的本原 

- [透过 rust 探索系统的本原：并发篇](./rust-to-system-essence-concurrent.md)
- [透过 rust 探索系统的本原：安全篇](./rust-to-system-essence-safety.md)

### Rust 编译器专题
- [如何为 Rust 语言做贡献 | Part 2](./contribute_to_the_rust_part2.md)