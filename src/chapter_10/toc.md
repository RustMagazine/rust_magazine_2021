# 十月刊（October）

## 发刊通告

- [发刊通告](./announce.md)

## Rust 资讯
- [官方动态](./lang.md)
- [社区热点](./hots.md)
- [活动回顾](./events.md)
- [本月招聘](./jobs.md)
- [聚焦Rust基金会新成员 | 为什么要加入 Rust 基金会 ](./rust-foundation.md)
- [【Linux Plumbers 大会总结】Rust 和 GCC 整合的两种方式](./rust-gcc-two-way.md)
- [P99 Conf Talk 汇总 |  Rust 在高性能低延迟系统中的应用](./p99conf.md)
- [《Rust 安全编码规范》 初稿发布](./rust-coding-guidelines.md)

## 外刊评论

- [知名项目进展报告](./foreign_journal_review.md)
- [Rust Async trait 更新与多线程](./async-trait.md)
- [Rust调研与内存安全](./rust-mem-safety.md)
- [对外输出 Mocks 接口 和 Tokio 的高延展性服务器设计](./mocks-and-tokio.md)

## Rust 生产实践

- [Datenlord |在 Rust 中管理 RDMA 内存 ](./rdma-memory-management-in-rust.md)
- [SecondState | Dapr的一个轻量安全可移植且高性能的Runtime](./dapr.md)
- [Dropbox | 为什么我们要为 Capture 构建自定义 Rust 库](./dropbox.md) 


## 学习园地

### 新手入门

- [学习资源](./learn.md)
- [【区块链】Solana 开发笔记 Part 1](./solana-learn-part1.md)
- [Rust 中常见的新手错误和不良实践：不良习惯](./bad-practice.md)

### 原理与工程实践

- [基于编译器插件定制 clippy lint](./rustc_plugin_custom_lints.md)

## 开源项目巡礼

- [Rust 生态观察｜ SeaORM: 要做 Rust 版本的 ActiveRecord  ](./seaorm.md)
- [`Delicate` 为什么从 `actix-web` 迁移到 `poem`](./delicate.md)
- [GitHub 趋势榜](./github_trending.md)
- [推荐项目 ｜ 基础工具库](./tool_libs.md)

##  Rust 与 安全

- [Rust生态安全漏洞总结系列 | Part 4](./rust-security-part4.md)
